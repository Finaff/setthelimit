...
(function () {
  ...
  // ---------- encoding ----------
  /* share links start with the content's major version ("5"), so a link made for another set of propositions is ignored, never misread */
  const LV = ((String(C.version || '').match(/^v(\d+)/) || [])[1] || '');
  const enc = (answers) => LV + ITEMS.map((it) => { const v = answers[it.id]; if (v === null) return '--'; if (typeof v !== 'number') return '__'; return v.toString(36).padStart(2, '0'); }).join('');
  function dec(str) {
    if (!str || str.length !== LV.length + ITEMS.length * 2 || str.slice(0, LV.length) !== LV) return null; /* a link from another item set is ignored, not misread */
    str = str.slice(LV.length); const out = {}; let any = false;
